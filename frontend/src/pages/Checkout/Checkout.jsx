...
const Checkout = () => {
  ...
  if (!query || !adults || !children) {
    return <Navigate to={'/buytickets'} />;
  }
  ...
  const { data: getTicket } = useFetch(
    `http://localhost:3000/tours/api/tickets?query=${query}`
  );

  const handleClick = async (e) => {
    try {
      const userFormDet = JSON.parse(localStorage.getItem('bookTickets'));
      ...
      const payload = {
        ...
        visitorCountry: userFormDet.visitorCountry,
        city: userFormDet.city,
        place: userFormDet.place,
        country: userFormDet.country,
        adultVisitors: userFormDet.adultVisitors,
        childrenVisitors: userFormDet.childrenVisitors,
        noOfFemaleAdult: userFormDet.noOfFemaleAdult,
        noOfFemalechildren: userFormDet.noOfFemalechildren,
        noOfMaleAdult: userFormDet.noOfMaleAdult,
        noOfMaleChildren: userFormDet.noOfMaleChildren,
        nationality: userFormDet.nationality,
        visitDate: userFormDet.date,
        email: userFormDet.email,
        phone: userFormDet.phone,
        transactionDate: String(new Date().toLocaleDateString("de-DE"))
      };

      console.log(payload);
      const {
        data: { url },
      } = await axios.post('http://localhost:3000/tours/api/payment', payload);
      // console.log(url);
      window.location = url;
    } catch (error) {
    ...
    //   return <Navigate to={`/buytickets?query=${query}`} />
    }
  };
      ...
      <div className='checkout'>
        <div className='checkout-header'>
          <div className='ticket-details'>
            <h3>
              {result[0] && result[0].place}: {result[0] && result[0].tag}
            </h3>
            <span>
              <p>
                {result[0] && result[0].city}, {result[0] && result[0].country}
              </p>
              |
              <p>
        ...
        <section className='booking-details-section'>
          ...
          <div className='booking-details-right'>
              ...
              <div className='inner'>
                <div className='booking-summary'>
                  ...
                  <div className='adult-ticket'>
                    <span className='ticket' id='adultTicket' data-value={2}>
                      Adult ({adults} Tickets)
                    </span>
                    <span
                      className='amount'
                      ...
                      data-value={900}
                    >
                      Rs. {getTicket.price} x {adults}
                    </span>
                  </div>
                  <div className='children-ticket'>
                    <span className='ticket' id='childrenTicket' data-value={2}>
                      Children ({children} Tickets)
                    </span>
                    <span
                      className='amount'
                      id='childrenTicketAmount'
                      data-value={900}
                    >
                      Rs. {getTicket.price} x {children}
                    </span>
                  </div>
                  <div className='convenience-fee'>
                    ...
                    <span id='convinienceFee' data-value={122}>
                      Rs. 122
                    </span>
                  </div>
                  <hr />
                  <div className='sub-total'>
                    <span>Sub Total</span>
                    <span id='subtotal' data-value={1022}>
                      Rs.{' '}
                      {getTicket.price * (Number(adults) + Number(children)) +
                        122}
                    </span>
                  </div>
                </div>
            ...
            <div className='ticket-type'>
              ...
              <p>
                Show the m-ticket QR Code on your mobile to enter to the place.
              </p>
            </div>
